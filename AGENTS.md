...
语音播报 TTS 应优先使用免 Key的 Edge Neural 中文音色，默认 `zh-CN-XiaoxiaoNeural`，先尝试 `msedge-tts`，失败或空音频时可尝试 Python `edge-tts` CLI，再回退 macOS `say` 或其他本地兜底；云端高质量 TTS 只能作为可选增强，不得成为现场语音功能的前置条件。
...
普通语音按钮只负责把识别结果填入输入框并等待用户手动发送；不得在转写完成后自动发送聊天或启动设计工作流。只有“小诺同学”语音唤醒模式可以继续自动执行明确的语音控制、页面操作或 quick design 路线；小诺唤醒态的普通问答应直接在主聊天区追加用户语音气泡并发送 `user_msg` with `voice:true`，不应再把识别文本塞到输入框等待确认。
小诺控制快速设计应优先走前端确定性命令路由：打开/关闭快速设计、下一步、上一步、启动设计，以及按当前步骤可见选项做疾病、靶点、机制、表位的模糊匹配；选择命令只选中/高亮，不自动跳下一步，除非用户明确说“然后下一步”。该控制链路不得使用大模型自由分类来决定 quick design route。
