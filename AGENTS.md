...
本项目每次完成任何文件修改后，都必须创建本地 Git commit 作为回滚点，推送当前工作分支到 GitHub 远端，并默认按公司服务器本地打包上传流程同步部署到公司服务器、验证公司服务器健康检查和构建版本号；默认不得上传阿里云服务器、替换阿里云线上版本、重启阿里云服务或执行阿里云发布验证。只有用户在当前任务中明确要求“部署到阿里云、发布到阿里云、上传阿里云服务器、更新阿里云线上版本”时，才按阿里云打包上传流程部署阿里云；当前固定目标环境分为公司服务器默认路线和阿里云服务器显式路线。
