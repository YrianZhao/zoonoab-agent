...
主页桌面布局应保持类似 ChatGPT/Gemini 的窄内容列和可收起左侧栏；左侧栏必须提供显式隐藏/显示按钮，`New Session` 固定在页面右上角并使用方框编辑图标，构建版本号显示在空状态提示文字下方居中，首页自然语言提示词不得包含“演示”或末尾句号。用户在任务流运行中也必须能随时收起或展开左侧栏，任务更新和新建会话不得强制改变桌面端侧栏开合状态；快速设计弹窗必须按当前侧栏状态适配右侧可用工作区。
