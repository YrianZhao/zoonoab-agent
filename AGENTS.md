...
公司服务器 `17010` 端口保留给从阿里云冻结下来的稳定展示版，默认根目录固定为 `/home/ajifang/zoonoab/zoonoab-agent-stable-17010`，并必须使用独立的 `releases/`、`current`、`shared/`、日志、运行数据、语音配置和 tmux session `zoonoab-agent-stable-17010`；启动时监听 `PORT=17010`。日常本地开发版部署、上传公司服务器、更新线上版本等默认操作只能触碰 `/home/ajifang/zoonoab/zoonoab-agent-10090`、`PORT=10090` 和 tmux session `zoonoab-agent-10090`，不得删除、覆盖、重建、重启或切换 `zoonoab-agent-stable-17010` 的任何目录、软链接、运行数据、端口或 session。只有用户在当前任务中明确提到“稳定版、17010、阿里云稳定版、替换稳定版”时，才允许操作稳定版目录；稳定版部署应优先使用已冻结的阿里云稳定版备份、稳定分支或 `aliyun-stable-*` tag，不得把当前开发分支 `HEAD` 误部署为稳定版。
