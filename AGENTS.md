...
Render 部署使用仓库根目录的 `render.yaml` Blueprint：Web Service 名称 `zoonoab-agent`，Node 运行时，`buildCommand: npm ci`，`startCommand: npm start`，健康检查 `/api/health`，并挂载持久磁盘到 `/var/data`。Render 默认使用服务端本地 ASR：`VOICE_ASR_PROVIDER=local`、`LOCAL_ASR_AUTO_START=1`、`LOCAL_ASR_BOOTSTRAP=1`，并将 `LOCAL_ASR_VENV_DIR`、`LOCAL_ASR_CACHE_DIR`、`MODELSCOPE_CACHE`、`HF_HOME`、`TORCH_HOME`、`PIP_CACHE_DIR` 指向 `/var/data`；首次部署或磁盘为空时允许运行时自动安装 ASR 依赖和下载模型，后续复用持久磁盘。`ASSISTANT_CHAT_API_KEY` 在 Render Dashboard 中填写，不能写入仓库。
