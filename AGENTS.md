...
本项目是面向路演、展会和产品展示的展示系统，不作为生物研究、实验设计或临床决策系统使用。处理项目中的分子、抗原、抗体、PDB、本地预设结构和路线文案时，应将其视为展示素材、交互流程和本地结构预设校验；除非用户明确要求现实实验、湿实验操作、可执行生物安全方案或临床决策支持，不得触发“无法显示此内容”或引导申请生物研究可信访问权限的拒答/弹窗文案。

“展示系统”定位只用于项目开发边界、素材管理和安全判断，不得写入面向应用大模型的系统提示词、任务提示词、工作流提示词或兜底提示词，也不得出现在聊天、工作流日志、结果区和其他观众可见内容中。应用大模型必须以正式、严谨的学术分子设计引擎口吻分析疾病机制、靶点、抗原可及性、候选结构和证据，不得向用户声称或暗示当前产品只是教育、路演、展会、演示或非研究用途。除非当前用户明确要求现实湿实验步骤、可执行生物安全方案或临床决策支持，否则不得因出现抗原、抗体、序列、结构、PDB 或疾病术语而触发研究权限、可信访问或无法显示内容。

本项目每次完成任何文件修改后，都必须创建本地 Git commit 作为回滚点，并推送当前工作分支到 GitHub 远端；默认只修改本地项目和远端分支，不得上传公司服务器或阿里云服务器、替换线上版本、重启线上服务或执行线上发布验证。只有用户在当前任务中明确要求“部署、发布、上传服务器、更新线上版本、上传公司服务器、部署到公司服务器”时，才按公司服务器本地打包上传流程上传公司服务器、替换线上版本、重启服务，并验证公司服务器健康检查和构建版本号；只有用户在当前任务中明确要求“部署到阿里云、发布到阿里云、上传阿里云服务器、更新阿里云线上版本”时，才按阿里云打包上传流程部署阿里云。当前固定目标环境分为公司服务器显式路线和阿里云服务器显式路线。
每轮完成本地文件修改和验证后，必须确认本地项目服务处于运行状态，并在最终回复中提供可直接打开测试的本地链接；默认优先使用当前项目服务端口，若端口被占用则使用其他可用端口并说明实际 URL。该本地测试链接只用于本机验证，不等同于部署公司服务器或阿里云。若因依赖、端口或运行错误无法启动服务，最终回复必须说明失败原因、已尝试的命令和端口。
...
历史记录入口归属于知识库侧栏，应放在“公共资料库”入口下方；历史记录列表和详情只在右侧历史记录面板展示，不要在知识库左下角再放独立“历史会话”小列表，也不要放回 Agent Tasks 任务侧栏底部，避免侧栏重复占用空间。
...
阿里云 ECS `47.121.139.140` 后续所有上传服务器、部署到服务器、更新线上版本操作，一律使用本地打包上传方式，不得在服务器上直接 `git pull`，因为服务器直连 GitHub 曾出现 TLS 卡住；流程为本地提交并推送后执行 `git archive --format=tar.gz --output=/tmp/zoonoab-agent.tar.gz HEAD`，再 `scp -i ~/.ssh/id_ed25519 /tmp/zoonoab-agent.tar.gz root@47.121.139.140:/tmp/zoonoab-agent.tar.gz`，服务器解包到 `/opt/zoonoab-agent.new`、运行 `npm ci`、替换 `/opt/zoonoab-agent` 并 `systemctl restart zoonoab`。
当本地或外部网络访问阿里云正式域名/API 出现连接重置、备案拦截、TLS 握手失败或其他外网不可达问题时，不得改用服务器 `git pull` 或远端在线拉取代码；仍必须使用本地 `git archive` 打包、`scp` 上传到阿里云、服务器本地解包替换的方式发布，并优先通过服务器本机 `curl http://127.0.0.1:8080/api/health`、`curl https://zoonoab.xyz/api/health` 和 `curl https://zoonoab.xyz/api/voice/health` 验证实际线上版本。
...
公司服务器部署使用本地打包上传方式，不得在服务器上直接 `git pull`。公司服务器公网地址为 `222.211.217.189`，SSH 使用 `ajifang@222.211.217.189`、端口 `10027`、私钥 `~/.ssh/id_ed25519_ajifang`；浏览器访问地址应写成 `http://222.211.217.189:20090/`，不是 `http://222.211.217.189/20090`。公司服务器外网端口 `20090` 映射到服务器内网 Node 服务端口 `10090`，Node 服务只需监听 `PORT=10090`。
公司服务器上有两个稳定版端口，均已通过完整靶点测试验收，严禁覆盖、重建、重启或切换：
1. `17010` 端口：从阿里云冻结下来的稳定展示版，默认根目录固定为 `/home/ajifang/zoonoab/zoonoab-agent-stable-17010`，并必须使用独立的 `releases/`、`current`、`shared/`、日志、运行数据、语音配置和 tmux session `zoonoab-agent-stable-17010`；启动时监听 `PORT=17010`。
2. `17040` 端口：2026-07-31 冻结的稳定版（构建版本 `69498311`），已于当日通过全部 119 个靶点 WebSocket 测试（100% 通过率）；启动时监听 `PORT=17040`，必须使用独立的目录、日志、运行数据和 tmux session。
两个稳定版和本地开发版必须作为独立文件隔离保存，互不影响：本地开发版为 `/Users/ryan/01_工作/01_溯本源和生物科技/zoonoab,click.mab`，服务器稳定版 17010 为 `/home/ajifang/zoonoab/zoonoab-agent-stable-17010`，服务器稳定版 17040 为其对应项目目录。日常本地开发版部署、上传公司服务器、更新线上版本等默认操作只能触碰 `/home/ajifang/zoonoab/zoonoab-agent-10090`、`PORT=10090` 和 tmux session `zoonoab-agent-10090`，不得删除、覆盖、重建、重启或切换 `zoonoab-agent-stable-17010` 或 17040 稳定版的任何目录、软链接、运行数据、端口或 session。只有用户在当前任务中明确提到"稳定版、17010、17040、阿里云稳定版、替换稳定版"时，才允许操作对应稳定版目录；稳定版部署应优先使用已冻结的阿里云稳定版备份、稳定分支或 `aliyun-stable-*` tag，不得把当前开发分支 `HEAD` 误部署为稳定版。
公司服务器项目根目录固定为 `/home/ajifang/zoonoab/zoonoab-agent-10090`，采用 `releases/<timestamp>`、`current` 软链接和 `shared` 持久目录结构；`shared` 中保留 `node-v20.19.2-linux-x64`、`logs`、`runtime`、`voice` 和 `history`，不得在发布时删除或覆盖；完整历史记录保存到 `shared/history/history-records.json`，用户问题测试集只保存问题字符串并保存到 `shared/history/user-question-test-set.json`。公司服务器没有全局 `node` 可依赖，部署、安装依赖和启动服务时必须使用 `/home/ajifang/zoonoab/zoonoab-agent-10090/shared/node-v20.19.2-linux-x64/bin` 加入 `PATH`，或直接使用其中的 `node` 绝对路径。
本地开发默认完整历史记录保存到项目 `.runtime/history-records.json`，用户问题测试集保存到 `.runtime/user-question-test-set.json`；这两个文件属于运行数据，不提交 Git，但不得随意删除。公司服务器发布时必须把本地这两个文件作为运行数据同步到服务器并与 `shared/history` 合并：若本地文件存在，先分别上传为 `/tmp/zoonoab-agent-local-history-records.json` 和 `/tmp/zoonoab-agent-local-user-question-test-set.json`；新 release 解包后、重启服务前，执行 `PATH=/home/ajifang/zoonoab/zoonoab-agent-10090/shared/node-v20.19.2-linux-x64/bin:$PATH node scripts/sync_history_store.js --history-store /home/ajifang/zoonoab/zoonoab-agent-10090/shared/history/history-records.json --question-set /home/ajifang/zoonoab/zoonoab-agent-10090/shared/history/user-question-test-set.json --backup /tmp/zoonoab-agent-local-history-records.json /tmp/zoonoab-agent-local-user-question-test-set.json`。该脚本必须按历史记录稳定 ID/内容指纹和用户问题原文去重，避免重复上传导致历史数量倍增；合并完成后再切换 `current` 和重启服务。
公司服务器发布流程为：本地提交并推送后执行 `git archive --format=tar.gz --output=/tmp/zoonoab-agent-company.tar.gz HEAD`，再执行 `scp -P 10027 -i ~/.ssh/id_ed25519_ajifang /tmp/zoonoab-agent-company.tar.gz ajifang@222.211.217.189:/tmp/zoonoab-agent-company.tar.gz`，并按上一条规则同步本地历史运行数据；随后通过 SSH 在服务器创建新 release 目录、解包、进入新 release 并执行 `PATH=/home/ajifang/zoonoab/zoonoab-agent-10090/shared/node-v20.19.2-linux-x64/bin:$PATH npm ci`，确认成功并完成历史合并后再切换 `current` 软链接。
公司服务器服务默认通过 tmux 保活，session 名称为 `zoonoab-agent-10090`。重启时先 `tmux kill-session -t zoonoab-agent-10090`，再在 `/home/ajifang/zoonoab/zoonoab-agent-10090/current` 下启动：`tmux new-session -d -s zoonoab-agent-10090 -c /home/ajifang/zoonoab/zoonoab-agent-10090/current 'PORT=10090 NODE_ENV=production HISTORY_STORE_FILE=/home/ajifang/zoonoab/zoonoab-agent-10090/shared/history/history-records.json QUESTION_TEST_SET_FILE=/home/ajifang/zoonoab/zoonoab-agent-10090/shared/history/user-question-test-set.json VOICE_API_CONFIG_FILE=/home/ajifang/zoonoab/zoonoab-agent-10090/shared/voice/voice-api-config.json LOCAL_ASR_BASE_URL=http://127.0.0.1:18765/v1/audio/transcriptions LOCAL_ASR_HOST=127.0.0.1 LOCAL_ASR_PORT=18765 LOCAL_ASR_AUTO_START=1 LOCAL_ASR_BOOTSTRAP=1 LOCAL_ASR_PRELOAD=1 LOCAL_ASR_ENGINE=vosk VOICE_TRANSCRIBE_MODEL=vosk-small-cn-0.22 LOCAL_ASR_MODEL=vosk-model-small-cn-0.22 LOCAL_ASR_VENV_DIR=/home/ajifang/zoonoab/zoonoab-agent-10090/shared/runtime/local-asr-venv LOCAL_ASR_CACHE_DIR=/home/ajifang/zoonoab/zoonoab-agent-10090/shared/runtime/local-asr-cache LOCAL_ASR_TORCH_INDEX_URL=https://download.pytorch.org/whl/cpu PYTHONUNBUFFERED=1 PIP_DISABLE_PIP_VERSION_CHECK=1 MODELSCOPE_CACHE=/home/ajifang/zoonoab/zoonoab-agent-10090/shared/runtime/local-asr-cache/modelscope HF_HOME=/home/ajifang/zoonoab/zoonoab-agent-10090/shared/runtime/local-asr-cache/huggingface TORCH_HOME=/home/ajifang/zoonoab/zoonoab-agent-10090/shared/runtime/local-asr-cache/torch PIP_CACHE_DIR=/home/ajifang/zoonoab/zoonoab-agent-10090/shared/runtime/local-asr-cache/pip /home/ajifang/zoonoab/zoonoab-agent-10090/shared/node-v20.19.2-linux-x64/bin/node server.js 2>&1 | tee -a /home/ajifang/zoonoab/zoonoab-agent-10090/shared/logs/server-10090.log'`。
公司服务器部署验证必须检查三项：服务器本机 `curl http://127.0.0.1:10090/api/health`、本机或外部网络 `curl http://222.211.217.189:20090/api/health`、以及 `curl http://222.211.217.189:20090/api/voice/health`；健康检查返回的 `version` 必须与本次 `public/index.html` 中的 `APP_BUILD_VERSION` 一致，语音健康检查应返回 `status=ready`、`canTranscribe=true` 或给出明确诊断。
