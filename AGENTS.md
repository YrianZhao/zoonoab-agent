...
工作流展示支持“跳过思考”按钮：点击后应让当前整次工作流进入快速展示模式，压缩后续服务端延迟、前端打字和断线 fallback 延迟，并按约 30 秒的演示节奏展示到最终结果和 3D 结构；不得瞬间刷完、不得取消工作流，也不得直接跳到最终结果。快速模式只影响当前运行，`done`、取消、错误或新任务开始时必须重置。
