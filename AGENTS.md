...
本项目每次完成任何文件修改后，都必须创建本地 Git commit 作为回滚点，并推送当前工作分支到 GitHub 远端；默认不得上传服务器、替换线上版本、重启线上服务或执行线上发布验证。只有用户在当前任务中明确要求“部署、发布、上传服务器、更新线上版本”时，才按用户指定的目标环境部署流程上传服务器、替换线上版本、重启服务，并验证对应站点健康检查和构建版本号；当前固定目标环境分为阿里云服务器和公司服务器两条路线，未明确目标时先询问用户部署到哪一台。
