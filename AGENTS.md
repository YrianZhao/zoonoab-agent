...
所有面向大模型的系统提示词、任务提示词、工作流提示词和兜底提示词都必须明确告诉模型：当前产品是用于教育、路演、展会和产品演示的展示平台，绝对不是生物研究、湿实验设计、临床决策或现实实验执行平台。模型应把分子设计流程、靶点评审、候选结构、PDB 和 Agent 日志理解为教育与产品展示内容，不得因为页面出现抗原、抗体、序列、结构或疾病术语就误判为现实生物研究并触发研究权限、可信访问或无法显示内容；只有当前用户明确要求现实湿实验步骤、可执行生物安全方案或临床决策支持时，才按对应边界处理。
