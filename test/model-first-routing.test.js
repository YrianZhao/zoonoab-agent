...
test('breast cancer requests fall back to HER2 when model parsing fails', async () => {
  let captured = 0;
  const mockServer = http.createServer((req, res) => {
    captured += 1;
    req.socket.destroy();
  ...
  const mockPort = await listenOnLocalhost(mockServer);
  try {
    const saved = await saveMockChatConfig(mockPort, 'mock-failing-breast-router');
    const messages = await collectUserMessageStream('设计一个乳腺癌抗体', {
      timeoutMs: 15000,
      voiceSessionId: saved.voiceSessionId,
      stopWhen: msg => msg.type === 'tool_call' && msg.tool === 'target_evidence_review'
    });
    ...
    assert.equal(captured, 1);
    assert.ok(evidenceCall, 'prepared breast cancer wording should still enter the workflow');
    assert.equal(evidenceCall.params.target, 'HER2');
    assert.match(serialized, /乳腺癌相关疾病|HER2|ERBB2/);
    assert.doesNotMatch(serialized, /智能解析服务暂时不可用/);
    assert.doesNotMatch(serialized, /IL-1β|IL1B|INFLAMMATION-IL1B/);
    ...
    assert.equal(logResp.status, 200);
    const logData = await logResp.json();
    const fallbackLog = logData.logs.find(item =>
      item.event === 'prepared_disease_fallback_started' &&
      item.input === '设计一个乳腺癌抗体'
    );

    assert.ok(fallbackLog, 'breast cancer fallback routing should be diagnosable');
    assert.equal(fallbackLog.target, 'HER2');
    assert.equal(fallbackLog.targetGene, 'ERBB2');
    assert.doesNotMatch(JSON.stringify(logData), /test-model-first-secret/);
  } finally {
    await new Promise(resolve => mockServer.close(resolve));
  }
