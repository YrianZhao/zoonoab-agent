...
// ─── Oligomer-aware entity counting (additive) ───
// These helpers classify how many distinct antigen molecules and antibody
// units are present in a complex, so that "one antigen -> many antibodies"
// and "one antibody -> many antigens" can be detected without flagging
// native oligomers (e.g. TNF trimer, VEGF dimer) whose chains touch.

function splitHeavyByChain(records, chains) {
  const allowed = new Set(splitChainList(chains));
  const map = {};
  for (const record of records || []) {
    if (record.isHeavy === false) continue;
    if (!allowed.has(record.chain)) continue;
    if (!map[record.chain]) map[record.chain] = [];
    map[record.chain].push(record);
  }
  return map;
}

function minInterChainDistance(recordsA, recordsB, cellSize) {
  const heavyA = (recordsA || []).filter(record => record.isHeavy !== false);
  const heavyB = (recordsB || []).filter(record => record.isHeavy !== false);
  if (!heavyA.length || !heavyB.length) return Infinity;
  const size = Math.max(0.5, Number(cellSize) || 6);
  const grid = buildSpatialGrid(heavyA, size);
  let minDistanceSq = Infinity;
  for (const b of heavyB) {
    const cell = b.xyz.map(value => Math.floor(value / grid.cellSize));
    for (let dx = -1; dx <= 1; dx += 1) {
      for (let dy = -1; dy <= 1; dy += 1) {
        for (let dz = -1; dz <= 1; dz += 1) {
          const bucket = grid.cells.get(gridKey(cell[0] + dx, cell[1] + dy, cell[2] + dz));
  ...
  return Number.isFinite(minDistanceSq) ? Math.sqrt(minDistanceSq) : Infinity;
}

function clusterChainsByContact(perChainRecords, gapThreshold) {
  const gap = Number(gapThreshold) || 6;
  const chains = Object.keys(perChainRecords || {}).filter(chain => perChainRecords[chain].length);
  const parent = {};
  for (const chain of chains) parent[chain] = chain;
  function find(x) {
    while (parent[x] !== x) {
  ...
  for (let i = 0; i < chains.length; i += 1) {
    for (let j = i + 1; j < chains.length; j += 1) {
      const d = minInterChainDistance(perChainRecords[chains[i]], perChainRecords[chains[j]], gap);
      if (d <= gap) union(chains[i], chains[j]);
    }
  }
  const groups = {};
  for (const chain of chains) {
    const root = find(chain);
    if (!groups[root]) groups[root] = [];
    groups[root].push(chain);
  }
  return Object.values(groups).map(group => ({ chains: group.slice().sort() }));
}

// Each contacting cluster counts as one antibody unit:
// a Fab heavy+light pair forms one cluster, a lone VHH chain forms one cluster.
function countAntibodyUnits(perChainRecords, format, gapThreshold) {
  const units = clusterChainsByContact(perChainRecords, gapThreshold);
  return { unitCount: units.length, units };
}

function analyzeComplexEntities(analysis, gapThreshold) {
  const working = analysis && analysis._workingSet;
  const roles = analysis && analysis.roles;
  if (!working || !working.records || !roles || !roles.antigenChains || !roles.antibodyChains) {
    return null;
  }
  const gap = Number(gapThreshold) || 6;
  const antigenByChain = splitHeavyByChain(working.records, roles.antigenChains);
  const antibodyByChain = splitHeavyByChain(working.records, roles.antibodyChains);
  const antigenChainsPresent = Object.keys(antigenByChain).filter(chain => antigenByChain[chain].length);
  const antibodyChainsPresent = Object.keys(antibodyByChain).filter(chain => antibodyByChain[chain].length);
  if (!antigenChainsPresent.length || !antibodyChainsPresent.length) return null;
  const antigenMolecules = clusterChainsByContact(antigenByChain, gap);
  const antibodyUnits = clusterChainsByContact(antibodyByChain, gap);
  return {
    gap,
    antigenMolecules,
    antibodyUnits,
    antigenMoleculeCount: antigenMolecules.length,
    antibodyUnitCount: antibodyUnits.length,
    multipleAntigen: antigenMolecules.length > 1,
    multipleAntibody: antibodyUnits.length > 1
  };
}
