...
function normalizeChainList(value, label) {
  ...
  for (const item of value) {
    const chain = String(item == null ? '' : item);
    if (chain.length > 1) {
      throw new DisplayPoseError('INVALID_CHAINS', label + ' contains a chain ID longer than one character', { chain });
    }
...
function buildSurfaceCandidates(antigenRecords, count, random) {
  const center = centerOf(antigenRecords);
...
function generateDisplayPose(options = {}) {
  try {
    ...
    const surfaceCount = clamp(Math.floor(Number(options.surfaceSampleCount) || 12), 8, 32);
    const surfaceCandidates = buildSurfaceCandidates(antigenHeavy, surfaceCount, random);
