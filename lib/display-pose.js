...
function buildSurfaceCandidates(antigenRecords, count, random, restrictChains) {
  ...
  let surfaceRecords = restrictChains && restrictChains.size > 0
    ? antigenRecords.filter(record => restrictChains.has(record.chain))
    : antigenRecords;
  const surfaceSourceRaw = surfaceRecords.length >= 3 ? surfaceRecords : antigenRecords;

  // Detect highly elongated antigens and restrict sampling to central region
  // to avoid placing antibody at extreme ends of stretched chains.
  const center = centerOf(surfaceSourceRaw);
  let surfaceSource = surfaceSourceRaw;
  if (surfaceSourceRaw.length >= 20) {
    let minXYZ = [Infinity, Infinity, Infinity];
    let maxXYZ = [-Infinity, -Infinity, -Infinity];
    for (const record of surfaceSourceRaw) {
      for (let axis = 0; axis < 3; axis++) {
        if (record.xyz[axis] < minXYZ[axis]) minXYZ[axis] = record.xyz[axis];
        if (record.xyz[axis] > maxXYZ[axis]) maxXYZ[axis] = record.xyz[axis];
      }
    }
    const spans = [maxXYZ[0]-minXYZ[0], maxXYZ[1]-minXYZ[1], maxXYZ[2]-minXYZ[2]];
    const maxSpan = Math.max(...spans);
    const minSpan = Math.min(...spans);
    const aspectRatio = minSpan > 1 ? maxSpan / minSpan : maxSpan;
    if (aspectRatio > 3.0 && maxSpan > 80) {
      // Elongated structure: find the longest axis and filter to central 60%
      let longAxis = 0;
      if (spans[1] > spans[longAxis]) longAxis = 1;
      if (spans[2] > spans[longAxis]) longAxis = 2;
      const projections = surfaceSourceRaw.map(record => record.xyz[longAxis] - center[longAxis]);
      ...
      surfaceSource = surfaceSourceRaw.filter(record => {
        const proj = record.xyz[longAxis] - center[longAxis];
        return proj >= p20 && proj <= p80;
      });
      if (surfaceSource.length < 10) surfaceSource = surfaceSourceRaw;
    }
  }
...
function comparePoses(left, right) {
  ...
  if (Math.abs(leftGap - rightGap) > 0.3) return rightGap - leftGap;
  // Compactness tiebreaker: prefer poses where antibody CoM is closer to antigen CoM
  const leftCoM = Number.isFinite(left.antibodyCoMDistance) ? left.antibodyCoMDistance : Infinity;
  const rightCoM = Number.isFinite(right.antibodyCoMDistance) ? right.antibodyCoMDistance : Infinity;
  if (Math.abs(leftCoM - rightCoM) > 1) return rightCoM - leftCoM;
  return 0;
}
  ...
  try {
    ...
    for (let surfaceIndex = surfaceStart; surfaceIndex < surfaceEnd; surfaceIndex += 1) {
      ...
      for (let twistIndex = 0; twistIndex < twistCount; twistIndex += 1) {
        ...
        // Compute antibody CoM distance from antigen CoM for compactness scoring
        const placedAntibodyCoM = add(centerOf(orientedHeavy), placement.placement);
        const antibodyCoMDistance = magnitude(sub(placedAntibodyCoM, antigenCenter));
        const pose = {
