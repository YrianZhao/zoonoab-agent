...
const MB = 1024 * 1024;
const DEFAULT_CACHE_DIR = path.resolve(__dirname, '..', '.runtime', 'structure-cache', 'v2');
...
function recordMatchesInput(record, input, directAccession = '') {
  ...
  const targetMatches = aliases.includes(requested) || names.includes(requested);
  if (input.targetGene) {
...
function entityChains(entity) {
  ...
  return uniqueStrings([
    ...(identifiers.auth_asym_ids || []),
    ...(identifiers.asym_ids || [])
  ]);
}
...
async function inspectRcsbCandidate(resolver, candidate, identity) {
  ...
  const antibodyChains = uniqueStrings(antibodyEntities.flatMap(item => entityChains(item.data)));
