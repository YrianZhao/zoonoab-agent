...
const MB = 1024 * 1024;
const DEFAULT_CACHE_DIR = path.resolve(__dirname, '..', '.runtime', 'structure-cache', 'v4');
...
function selectContactingAntigenChains(pdbText, antigenChains, antibodyChains, thresholdA = 8) {
  const antigens = uniqueStrings(antigenChains);
  const antibodies = new Set(uniqueStrings(antibodyChains));
  if (antigens.length <= 1 || !antibodies.size) return antigens;
  const antigenSet = new Set(antigens);
  const antigenAtoms = new Map(antigens.map(chain => [chain, []]));
  const antibodyAtoms = [];
  for (const line of String(pdbText || '').split(/\r?\n/)) {
    if (!line.startsWith('ATOM  ')) continue;
    const chain = (line[21] || '').trim();
    if (!antigenSet.has(chain) && !antibodies.has(chain)) continue;
    const atom = [
      Number.parseFloat(line.slice(30, 38)),
      Number.parseFloat(line.slice(38, 46)),
      Number.parseFloat(line.slice(46, 54))
    ];
    if (!atom.every(Number.isFinite)) continue;
    if (antibodies.has(chain)) antibodyAtoms.push(atom);
    else antigenAtoms.get(chain).push(atom);
  }
  if (!antibodyAtoms.length) return antigens;
  const thresholdSq = thresholdA * thresholdA;
  const grid = new Map();
  const cellKey = (x, y, z) => x + ',' + y + ',' + z;
  for (const atom of antibodyAtoms) {
    const cell = atom.map(value => Math.floor(value / thresholdA));
    const key = cellKey(cell[0], cell[1], cell[2]);
    if (!grid.has(key)) grid.set(key, []);
    grid.get(key).push(atom);
  }
  const contacting = antigens.filter(chain => antigenAtoms.get(chain).some(atom => {
    const cell = atom.map(value => Math.floor(value / thresholdA));
    for (let dx = -1; dx <= 1; dx += 1) {
      for (let dy = -1; dy <= 1; dy += 1) {
        for (let dz = -1; dz <= 1; dz += 1) {
          const nearby = grid.get(cellKey(cell[0] + dx, cell[1] + dy, cell[2] + dz)) || [];
          if (nearby.some(other => {
            const x = atom[0] - other[0];
            const y = atom[1] - other[1];
            const z = atom[2] - other[2];
            return x * x + y * y + z * z <= thresholdSq;
          })) return true;
        }
      }
    }
  ...
  return contacting.length ? contacting : antigens;
}
...
async function downloadRcsbCandidate(resolver, meta, identity) {
  ...
  for (const attempt of attempts) {
    try {
      ...
      const displayAntigenChains = selectContactingAntigenChains(pdb.text, antigenChains, antibodyChains);
      return {
        ...attempt,
        pdb,
...
function makeReadyRcsbContract(input, cacheKey, identity, meta, download, nowIso) {
  ...
  const representativeInterface = hasAntibody && download.antigenChains.length < download.sourceAntigenChains.length;
  ...
  return {
    ...
    coordinates: {
      ...
      sourceAntigenChains: download.sourceAntigenChains,
      sourceAntibodyChains: download.sourceAntibodyChains
    },
    pose: { kind: representativeInterface ? 'representative_interface' : (hasAntibody ? 'experimental_complex' : 'antigen_only'), geometryValidated: false },
    display: {
      grade: hasAntibody ? 'A' : 'B',
      interfaceDetail: hasAntibody
        ? (representativeInterface
          ? `从完整${biologicalText}中保留与当前单个抗体直接接触的抗原链 ${download.antigenChains.join(', ')} 和抗体链 ${download.antibodyChains.join(', ')}${coverageLabel}；未将远端对称拷贝重复显示。`
      ...
      structureTitle: hasAntibody
        ? `${input.requestedTarget} ${representativeInterface ? '实验结合界面' : '实验复合物结构'}`
      ...
      visualSummary: hasAntibody
        ? (representativeInterface
          ? `展示公开实验结构中一个完整抗体与其直接接触抗原链的代表性结合界面。`
      ...
      disclosure: hasAntibody
        ? (domainFragment
          ...
          : (representativeInterface
            ? '抗原身份、物种和链映射已按 UniProt 与 RCSB 实体交叉验证；当前为从 biological assembly 提取的单抗体代表性实验结合界面，不等同于完整天然多聚体展示。'
