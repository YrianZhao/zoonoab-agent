...
const DISEASE_AREA_PATTERN = /^(?:一个|一种|一类|针对|关于|面向|用于|治疗|预防|改善|控制|靶向|抗)?\s*(?:肥胖|减重|超重|糖尿病|2型糖尿病|一型糖尿病|二型糖尿病|高血糖|胰岛素抵抗|代谢综合征|脂肪肝|高血压|阿尔茨海默|老年痴呆|帕金森|肾病|慢性肾病|肝病|心衰|冠心病|动脉粥样硬化|卒中|中风|抑郁|焦虑|银屑病|红斑狼疮|克罗恩|溃疡性结肠炎|炎症性肠病|哮喘|过敏|鼻炎|湿疹|皮炎|肺癌|乳腺癌|胃癌|肝癌|胰腺癌|结直肠癌|实体瘤|肿瘤|癌症|感染|炎症|自身免疫|代谢病|代谢疾病|心血管疾病|神经退行性疾病|罕见病|obesity|overweight|weight loss|diabetes|diabetic|hyperglycemia|insulin resistance|metabolic syndrome|nash|nafld|hypertension|alzheimer'?s?|parkinson'?s?|psoriasis|lupus|crohn'?s?|ibd|asthma|allergy|eczema|cancer|tumou?r|infection|inflammation|autoimmune|cardiovascular disease|neurodegenerative disease)\s*(?:方向|相关疾病|疾病|治疗|抗体|候选|项目|场景|领域)?$/i;
const EXPLICIT_TARGET_CONTEXT_PATTERN = /(?:靶点|抗原|蛋白|受体|配体|因子|细胞因子|target|antigen|protein|receptor|ligand|cytokine)\s*(?:是|为|:|：)?|(?:针对|靶向|结合|中和|阻断|抗)\s*(?:human\s+)?(?:[A-Z0-9][A-Z0-9()._\-\/]{1,50}|[A-Za-z]{2,}[A-Za-z0-9()._\-\/]{0,50})/i;
...
function isDiseaseAreaTerm(target) {
  const value = String(target || '').trim();
  if (!value) return false;
  return DISEASE_AREA_PATTERN.test(value);
}
...
function isPlausibleTarget(target, sourceText = '') {
  ...
  if (isDiseaseAreaTerm(target)) return false;
  if (TARGET_LIKE_PATTERN.test(target)) return true;
  if (EXPLICIT_TARGET_CONTEXT_PATTERN.test(sourceText) && !DISEASE_AREA_PATTERN.test(target)) return true;
  return false;
}
