...
const BIOMEDICAL_CONTEXT_PATTERN = /抗体|单抗|单克隆|抗原|靶点|表位|蛋白|细胞|受体|配体|通路|疾病|治疗|肿瘤|癌|哮喘|过敏|炎症|自身免疫|类风湿|关节炎|感染|病原|细菌|真菌|病毒|疫苗|免疫|生物|分子|植物病原|肥胖|糖尿病|银屑病|阿尔茨海默|代谢|流感|神经氨酸酶|血凝素|pd-1|pd-l1|il-33|st2|her2|erbb2|tnf|egfr|vegf|cd3|cd20|bcma|mab|biolog|biomedical|therapeutic|tumou?r|cancer|asthma|allergy|inflammation|autoimmune|infection|pathogen|bacteria|fungal|viral|antigen|target|epitope|protein|receptor|ligand|immune|vaccine|antibody|monoclonal/;
const ANTIBODY_DESIGN_OBJECT_PATTERN = /抗体|单抗|单克隆抗体|候选抗体|抗体候选|纳米抗体|单域抗体|片段抗体|候选|分子|结合体|结合序列|候选序列|抗体序列|单抗序列|binder|antibody|monoclonal|mab|nanobody|vhh|fab|scfv/i;
const DESIGN_ACTION_PATTERN = /设计|生成|开发|构建|筛选|做|来一个|来一批|制备|获得|输出|design|generate|create|make|develop/i;
const DESIGN_REQUEST_PATTERN = /(设计|生成|开发|构建|筛选|做|来一个|来一批|制备|获得|输出|design|generate|create|make|develop).{0,100}(抗体|单抗|单克隆抗体|候选抗体|抗体候选|纳米抗体|单域抗体|片段抗体|候选|分子|结合体|结合序列|候选序列|抗体序列|单抗序列|binder|antibody|monoclonal|mab|nanobody|vhh|fab|scfv)|(抗体|单抗|单克隆抗体|mab|antibody|binder).{0,50}(设计|生成|开发|筛选|design|generate|develop)/i;
...
function parseCount(input, fallback = 10) {
  ...
  const chineseMatch = raw.match(/(?:设计|生成|开发|构建|筛选|做|来|获得|制备|给我|帮我|需要|输出)?\s*(一|二|两|三|四|五|六|七|八|九|十)\s*(?:个|条|组|支|款|种|份|候选)?\s*(?:高亲和力|高特异性|候选|抗体|分子|binder|antibody|nanobody|vhh|fab|scfv)/i) ||
...
function extractUnknownTarget(input) {
  ...
  const patterns = [
    ...
    /(?:设计|生成|开发|构建|筛选|做|来一个|来一批|制备|获得|输出)\s*(?:\d+\s*(?:个|条|组|支)?\s*)?(?:具有结合活性|结合活性|高亲和力|高特异性|中和性|候选|可打印|用于检测|用于治疗)?\s*(?:针对|靶向|结合|中和|阻断|抗)?\s*([^，。！？、,.!?;；:：\n]{2,60}?)(?:的)?(?:抗体|单抗|单克隆抗体|候选抗体|抗体候选|抗体序列|单抗序列|候选序列|结合序列|分子|binder|antibody|monoclonal|mab|nanobody|vhh|fab|scfv)/i,
...
function extractDiseaseIndication(input) {
  ...
  if (!raw || shouldSuppressDesignWorkflow(raw)) return '';
  const patterns = [
    /(?:请|帮我|为|给)?(?:为|给|针对|面向|用于治疗|治疗)\s*([^，。！？、,.!?;；:：\n]{2,40}?)(?:设计|生成|开发|构建|筛选|做|制备|获得)\s*(?:\d+|一|二|两|三|四|五|六|七|八|九|十)?\s*(?:个|条|组|支|款|份|候选)?\s*(?:抗体|候选抗体|抗体候选|分子|binder|antibody|nanobody|vhh|fab|scfv)/i,
    /(?:设计|生成|开发|构建|筛选|做|来一个|制备|获得)\s*(?:\d+\s*(?:个|条|组|支)?\s*)?(?:一个|一种|一类|某个|某种)?\s*(?:针对|靶向|面向|治疗|用于治疗|用于)?\s*([^，。！？、,.!?;；:：\n]{2,40}?)(?:的)?(?:抗体|候选抗体|抗体候选|分子|binder|antibody|nanobody|vhh|fab|scfv)/i,
    /(?:针对|靶向|面向|治疗|用于治疗|用于)\s*([^，。！？、,.!?;；:：\n]{2,40}?)(?:的)?(?:抗体|候选|分子|binder|antibody)/i
  ];
  for (const pattern of patterns) {
    const match = raw.match(pattern);
    if (!match) continue;
