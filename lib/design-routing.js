...
} = require('./local-structure-catalog');

const LOCAL_STRUCTURE_CATALOG = loadLocalStructureCatalog(path.resolve(__dirname, '..'));
const PREPARED_TARGET_ROUTE_ID_MAP = buildTargetRouteMapFromCatalog(LOCAL_STRUCTURE_CATALOG);
const PREPARED_ROUTE_TARGET_BY_ID = Object.fromEntries(
  catalogRouteEntries(LOCAL_STRUCTURE_CATALOG, { includeClientFallbackOnly: false })
    .filter(entry => entry && entry.routeId && entry.target)
    .map(entry => [entry.routeId, String(entry.target || '').trim()])
);

function preparedTargetAliasSegments(value) {
  ...
  const queue = [raw];
  const suffixPattern = /^(.*?)(?:\s*(?:receptor|protein|antigen|domain|fragment|subunit|ecd|e\d{1,2}|extracellular(?:\s+domain|\s+region)?))$/i;
  const parentheticalPattern = /^(.*?)\s*[\(\[\{][^()\[\]{}]+[\)\]\}]$/;
  while (queue.length) {
    const current = queue.shift();
    const suffixMatch = current.match(suffixPattern);
    if (suffixMatch && suffixMatch[1] && suffixMatch[1].trim() && !parts.has(suffixMatch[1].trim())) {
      parts.add(suffixMatch[1].trim());
      queue.push(suffixMatch[1].trim());
    }
    const parentheticalMatch = current.match(parentheticalPattern);
    if (parentheticalMatch && parentheticalMatch[1] && parentheticalMatch[1].trim() && !parts.has(parentheticalMatch[1].trim())) {
      parts.add(parentheticalMatch[1].trim());
      queue.push(parentheticalMatch[1].trim());
    }
    for (const segment of current.split(/[\/,，、;；|]/)) {
      const trimmed = segment.trim();
      if (trimmed && !parts.has(trimmed)) {
        parts.add(trimmed);
        queue.push(trimmed);
      }
    }
...
function isDisplayPoseRouteEntry(entry) {
  return /^display_pose_/i.test(String(entry && entry.routeId || ''));
}

function buildPreparedTargetCanonicalMap() {
  const canonicalMap = {};
  const allEntries = catalogRouteEntries(LOCAL_STRUCTURE_CATALOG, { includeClientFallbackOnly: false });
  // Sort entries: non-display_pose (hand-crafted profiles) first, then display_pose
  const entries = allEntries.slice().sort((a, b) => {
    const aDisplay = isDisplayPoseRouteEntry(a);
    const bDisplay = isDisplayPoseRouteEntry(b);
    if (aDisplay !== bDisplay) return aDisplay ? 1 : -1;
    return 0;
  });
  // Add the exact target key (not split into sub-segments) with overwrite capability
  const addTargetKey = (target, canonicalTarget, overwrite) => {
    const key = normalizeStructureCatalogKey(target);
    if (key && (!canonicalMap[key] || overwrite)) canonicalMap[key] = canonicalTarget;
  };
  // Add alias segments (split by /, commas, etc.) without overwriting existing entries
  const addAlias = (alias, canonicalTarget) => {
    for (const variant of preparedTargetAliasSegments(alias)) {
      const key = normalizeStructureCatalogKey(variant);
      if (!key) continue;
      if (!canonicalMap[key]) canonicalMap[key] = canonicalTarget;
    }
  };

  // Pass 1: Add exact target keys (non-display_pose can overwrite, display_pose cannot)
  for (const entry of entries) {
    if (!entry || entry.routeable === false) continue;
    const canonicalTarget = String(entry.target || '').trim();
    if (!canonicalTarget) continue;
    const canOverwrite = !isDisplayPoseRouteEntry(entry);
    addTargetKey(canonicalTarget, canonicalTarget, canOverwrite);
  }
  // Pass 2: Add aliases, promptLabels, and genes (never overwrite)
  for (const entry of entries) {
    if (!entry || entry.routeable === false) continue;
    const canonicalTarget = String(entry.target || '').trim();
    if (!canonicalTarget) continue;
    addAlias(canonicalTarget, canonicalTarget);
    addAlias(entry.promptLabel, canonicalTarget);
    for (const alias of Array.isArray(entry.aliases) ? entry.aliases : []) {
      addAlias(alias, canonicalTarget);
    }
    addAlias(entry.gene, canonicalTarget);
  }

  for (const [routeId, canonicalTarget] of Object.entries(PREPARED_ROUTE_TARGET_BY_ID)) {
    const aliasKey = normalizeStructureCatalogKey(routeId);
    if (aliasKey && !canonicalMap[aliasKey]) canonicalMap[aliasKey] = canonicalTarget;
  }
  for (const [aliasKey, routeId] of Object.entries(PREPARED_TARGET_ROUTE_ID_MAP)) {
    const canonicalTarget = PREPARED_ROUTE_TARGET_BY_ID[routeId];
    if (aliasKey && canonicalTarget && !canonicalMap[aliasKey]) canonicalMap[aliasKey] = canonicalTarget;
  }
  return canonicalMap;
}

const PREPARED_TARGET_CANONICAL_MAP = buildPreparedTargetCanonicalMap();

function preparedAliasIsSafeForTextSearch(alias) {
...
function buildPreparedTargetTextAliases() {
  const rows = [];
  const allEntries = catalogRouteEntries(LOCAL_STRUCTURE_CATALOG, { includeClientFallbackOnly: false });
  // Sort entries: non-display_pose first so their canonical names take priority
  const entries = allEntries.slice().sort((a, b) => {
    const aDisplay = isDisplayPoseRouteEntry(a);
    const bDisplay = isDisplayPoseRouteEntry(b);
    if (aDisplay !== bDisplay) return aDisplay ? 1 : -1;
  ...
  for (const entry of entries) {
    ...
    for (const alias of aliases) {
      for (const variant of preparedTargetAliasSegments(alias)) {
        ...
        if (!text || !preparedAliasIsSafeForTextSearch(text)) continue;
        rows.push({ text, key: normalizeStructureCatalogKey(text), canonical: canonicalTarget });
      }
    }
  }
  rows.sort((a, b) => b.key.length - a.key.length || b.text.length - a.text.length);
  // Deduplicate by text only, keeping the first (highest priority) canonical target
...
const PREPARED_TARGET_TEXT_ALIASES = buildPreparedTargetTextAliases();

function escapeRegExp(value) {
  return String(value || '').replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
}

function preparedAliasAppearsInText(alias, text) {
  const value = String(alias || '').trim();
  const raw = String(text || '');
  if (!value || !raw) return false;
  if (/^[\u4e00-\u9fff]+$/.test(value)) return raw.includes(value);
  const token = escapeRegExp(value).replace(/\s+/g, '\\s+').replace(/\\-/g, '[-\\s]?');
  return new RegExp('(^|[^A-Za-z0-9])' + token + '(?=$|[^A-Za-z0-9])', 'i').test(raw);
}

function preparedTargetFromText(text) {
  for (const row of PREPARED_TARGET_TEXT_ALIASES) {
    if (!row.key || row.key.length < 2) continue;
    if (preparedAliasAppearsInText(row.text, text)) return row.canonical;
  }
  return '';
}
...
const KNOWN_TARGET_ALIASES = [
  ...
  { canonical: 'B7-H6', patterns: [/\bb7\s*-\s*h\s*6\b/i, /\bb7h6\b/i, /\bncr3lg1\b/i] },
  ...
  { canonical: 'CD22', patterns: [/\bcd\s*-?\s*22\b/i, /\bsiglec\s*-?\s*2\b/i, /\bsiglec2\b/i, /b[\s-]*cell receptor cd22/i, /leu[\s-]*14/i] },
...
const DISEASE_INDICATION_PATTERN = /肥胖|糖尿病|银屑病|阿尔茨海默|老年痴呆|代谢病|代谢疾病|心肌炎|心肌病|心血管|高血脂|高脂血症|多动症|注意缺陷|注意缺陷多动障碍|多发性骨髓瘤|骨髓瘤|b-all|b all|b-?cell acute lymphoblastic leukemia|acute b lymphoblastic leukemia|b细胞急性淋巴细胞白血病|b 细胞急性淋巴细胞白血病|急性b淋巴细胞白血病|白血病|淋巴瘤|淋巴细胞白血病|髓系|AML|癌|肿瘤|哮喘|过敏|皮炎|湿疹|嗜酸性|炎症|炎症性肠病|溃疡性结肠炎|克罗恩|类风湿|关节炎|骨关节炎|骨质疏松|疼痛|慢性疼痛|红斑狼疮|狼疮|系统性红斑狼疮|重症肌无力|甲状腺眼病|graves|graves disease|thyroid eye disease|parkinson|parkinson's disease|帕金森|帕金森病|视神经脊髓炎|neuromyelitis optica|nmosd|感染|obesity|diabetes|psoriasis|alzheimer|myocarditis|cardiomyopathy|metabolic|cardiovascular|adhd|attention deficit|hyperactivity|multiple myeloma|myeloma|leukemia|lymphoma|myeloid|cancer|asthma|allergy|dermatitis|eczema|inflammation|arthritis|osteoarthritis|osteoporosis|pain|chronic pain|ulcerative colitis|crohn|ibd|lupus|sle|myasthenia|myasthenia gravis|gmg/i;
const CONCISE_DISEASE_TREATMENT_REQUEST_PATTERN = /^(?:请|帮我|给|来|想|需要)?\s*(?:治疗|针对|面向|用于治疗)\s*([^，。！？、,.!?;；:：\n]{2,24})\s*$/i;
const MEDICAL_ADVICE_CUE_PATTERN = /怎么|如何|什么|哪些|是否|是不是|能不能|可以吗|副作用|预后|诊断|分期|检查|用药|药物|药|指南|推荐|症状|原因|复发|转移|生存率|存活率/i;
...
function resolvePreparedTargetCanonical(value) {
  const segments = preparedTargetAliasSegments(value);
  for (const segment of segments) {
    const key = normalizeStructureCatalogKey(segment);
    if (key && PREPARED_TARGET_CANONICAL_MAP[key]) return PREPARED_TARGET_CANONICAL_MAP[key];
  }
  return '';
}
...
function normalizeTargetAlias(target) {
  ...
  const preparedCanonical = resolvePreparedTargetCanonical(value);
  if (preparedCanonical) return preparedCanonical;
  ...
  if (/^(b7-?h6|ncr3lg1)$/.test(compact)) return 'B7-H6';
  ...
  if (/^(cd-?22|siglec-?2|siglec2|bcellreceptorcd22|leu-?14)$/.test(compact)) return 'CD22';
...
function extractKnownTarget(input) {
  const raw = String(input || '');
  if (/(?:狗|犬|犬源|犬用|canine|dog)[^，。；;]{0,32}(?:\bNGF\b|nerve growth factor|神经生长因子)/i.test(raw) ||
      /(?:\bNGF\b|nerve growth factor|神经生长因子)[^，。；;]{0,32}(?:狗|犬|犬源|犬用|canine|dog)/i.test(raw)) {
...
function trimTargetCandidate(value) {
  ...
  target = target
    ...
    .replace(/(?:设计|生成|开发|构建|筛选|做|来一个|来一批|制备|获得|输出).*$/i, '')
...
function extractFrontLoadedTarget(input) {
  const raw = String(input || '').trim();
  if (!raw) return '';
  if (/A\s*β|A\s*beta|Abeta|Amyloid[- ]?beta|Amyloid[- ]?β/i.test(raw)) {
    return normalizeTargetAlias('Aβ');
  }
  if (/Immunoglobulin\s*E/i.test(raw)) {
    return normalizeTargetAlias('Immunoglobulin E');
  }
  if (/\bGJB2\b/i.test(raw)) {
    return normalizeTargetAlias('GJB2');
  }
  if (/\bMSTN\b/i.test(raw)) {
    return normalizeTargetAlias('MSTN');
  }
  if (/\bNTRK2\b/i.test(raw)) {
    return normalizeTargetAlias('NTRK2');
  }
  const preparedFromText = preparedTargetFromText(raw);
  if (preparedFromText) return preparedFromText;
  const patterns = [
    /^(?:请|帮我|给我|请为|帮我为|为)\s*(Aβ|Abeta|Amyloid[- ]?beta|Amyloid[- ]?β)(?=\s*(?:设计|生成|开发|构建|筛选|做|来一个|来一批|制备|获得|输出))/i,
    /^(?:请|帮我|给我|请为|帮我为|为)\s*(?:针对|靶向|面向|用于|用于治疗)?\s*(?:human\s+)?([A-Za-z0-9\u00C0-\u024F\u0370-\u03FF][A-Za-z0-9()._\-\/\s\u00C0-\u024F\u0370-\u03FF\u4e00-\u9fff]{1,60}?)(?=\s*(?:设计|生成|开发|构建|筛选|做|来一个|来一批|制备|获得|输出))/i,
    /^(?:请|帮我|给我|请为|帮我为|为)\s*(?:针对|靶向|面向|用于|用于治疗)?\s*(?:human\s+)?([A-Za-z0-9\u00C0-\u024F\u0370-\u03FF][A-Za-z0-9()._\-\/\s\u00C0-\u024F\u0370-\u03FF\u4e00-\u9fff]{1,60}?)(?=\s*(?:的)?\s*(?:单克隆抗体|单抗|候选抗体|抗体候选|抗体|分子|药物|治疗分子|binder|drug|medicine|monoclonal\s+antibody|antibody|nanobody|vhh|fab|scfv))/i
  ];
  for (const pattern of patterns) {
    const match = raw.match(pattern);
    if (!match) continue;
    const target = trimTargetCandidate(match[1]);
    if (pattern === patterns[0] && /(?:病毒|virus|pathogen|bacteria|bacterial|细菌|真菌)/i.test(target) && !extractKnownTarget(target).target) continue;
...
function extractExplicitTargetDeclaration(input) {
  ...
  const directExplicitMatch = raw.match(/(?:靶点|抗原|目标抗原|target|antigen)\s*(?:是|为|:|：|=|is)\s*([^\n，。！？、,.!?;；:：]{2,60})/i);
  if (directExplicitMatch) {
    const target = trimTargetCandidate(directExplicitMatch[1]);
    if (target && isPlausibleTarget(target, raw)) return target;
  }
  const patterns = [
    /^(?:请|帮我|给我|请给我|帮我为|请为|为)?\s*(Aβ|Abeta|Amyloid[- ]?beta|Amyloid[- ]?β)(?=\s*(?:设计|生成|开发|构建|筛选|做|来一个|来一批|制备|获得|输出)|\s*(?:单克隆抗体|单抗|候选抗体|抗体候选|抗体|分子|药物|治疗分子|binder|drug|medicine|monoclonal\s+antibody|antibody|nanobody|vhh|fab|scfv))/i,
...
function extractConciseDiseaseTreatmentIndication(input) {
  const raw = String(input || '').trim();
  if (!raw || shouldSuppressDesignWorkflow(raw) || MEDICAL_ADVICE_CUE_PATTERN.test(raw)) return '';
  const match = raw.match(CONCISE_DISEASE_TREATMENT_REQUEST_PATTERN);
  if (!match) return '';
  const value = trimTargetCandidate(match[1]);
  return isDiseaseIndication(value) ? value : '';
}
...
function extractDiseaseIndication(input) {
  ...
  return extractConciseDiseaseTreatmentIndication(raw);
}
...
function extractDesignRequest(input) {
  ...
  const frontLoadedTarget = extractFrontLoadedTarget(raw);
  ...
  const conciseDiseaseIndication = extractConciseDiseaseTreatmentIndication(raw);
  if (!hasDesignIntent && !(DESIGN_ACTION_PATTERN.test(raw) && ANTIBODY_DESIGN_OBJECT_PATTERN.test(raw)) && !hasKnownTargetDesignIntent && !conciseDiseaseIndication) {
    return { isDesignRequest: false, count: 10, target: '', blockTarget: null, abType: 'Fab' };
  }
  if (!known.target && !explicitTarget && !frontLoadedTarget && !hasBiomedicalContext(raw) && !/(病毒|病原|抗原|抗体|单抗|单克隆|protein|antigen|antibody|monoclonal|mab|virus|viral)/i.test(raw)) {
    return { isDesignRequest: false, count: 10, target: '', blockTarget: null, abType: 'Fab' };
  }
  const diseaseIndication = extractDiseaseIndication(raw) || conciseDiseaseIndication;
  const rawTarget = known.target || explicitTarget || frontLoadedTarget || extractUnknownTarget(raw) || diseaseIndication;
  const target = rawTarget ? normalizeTargetAlias(trimTargetCandidate(rawTarget)) : '';
  return {
