...
function buildPreparedTargetCanonicalMap() {
  const canonicalMap = {};
  const entries = catalogRouteEntries(LOCAL_STRUCTURE_CATALOG, { includeClientFallbackOnly: false });
  const addAlias = (alias, canonicalTarget, overwrite = false) => {
    for (const variant of preparedTargetAliasSegments(alias)) {
      const key = normalizeStructureCatalogKey(variant);
      if (!key) continue;
      if (!canonicalMap[key] || overwrite) canonicalMap[key] = canonicalTarget;
    }
  };
  ...
  for (const entry of entries) {
    ...
    addAlias(canonicalTarget, canonicalTarget, true);
...
function buildPreparedTargetTextAliases() {
  const rows = [];
  for (const entry of catalogRouteEntries(LOCAL_STRUCTURE_CATALOG, { includeClientFallbackOnly: false })) {
    if (!entry || entry.routeable === false) continue;
    const canonicalTarget = String(entry.target || '').trim();
    if (!canonicalTarget) continue;
  ...
  return rows.filter(row => {
    const key = row.text.toLowerCase() + '|' + row.canonical;
